...
import pandas as pd
import filter_replicate_script.list_PROBLEMATIC_positions as prob
import os
...
def filter_df(df, freq, coverage, base_count):
    ...
    """
    df = df.loc[(df['frequency'] >= freq) & (df['ref_base'] != df['read_base']) &
...
def filter(tsv1, tsv2, patient, timepoint, freq, coverage, base_count, protein_dict):
    ...
    for ind, row in merged_df.iterrows():
        ...
        # Usecase 4
            ...
            # merged_df.drop(ind, inplace=True)  # Drop row
            # Usecase 6
            else:
